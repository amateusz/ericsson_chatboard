...
@click.option(
    "--port",
    default="/dev/ttyUSB0",
    help="serial port with a chatboard",
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="play sounds when typing. requires files type{1-3}.wav",
)
def main(port, sound):
    import serial
            ...
            for extended_key in extended_keys:
                if extended_key in key:
                    ...
                    if index == 2:  # CKPD
                        ...
                        anti_repeat = (
                            True
                            if (value.find("1c") == 0 or value.find("0c") == 0)
                            else False
                        )
                        if anti_repeat:
                            value = value[2:]
